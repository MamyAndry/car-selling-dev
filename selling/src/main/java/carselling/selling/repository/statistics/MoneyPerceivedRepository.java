...
public interface MoneyPerceivedRepository extends CrudRepository<Vente, String> {
    ...
    @Query(nativeQuery = true,value = "SELECT * FROM f_get_profit_user_per_month() WHERE id_user_result LIKE :id")
    ...
    @Query(nativeQuery = true,value = "SELECT * FROM f_get_profit_user_per_month WHERE year = :year AND id_user_result LIKE :id")
    public List<ProfitPerYear> getProfitOfYear(@Param(value = "id") String id,@Param(value = "year") int year);

    @Query(nativeQuery = true,value = "SELECT * FROM f_get_fund_user_per_month()")
    public List<FundPerMonthPerYear> getFundVariationPerMonthPerYear();

    @Query(nativeQuery = true,value = "SELECT * FROM f_get_fund_user_per_month() WHERE year = :year")
